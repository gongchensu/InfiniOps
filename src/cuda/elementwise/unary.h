#ifndef INFINI_OPS_CUDA_ELEMENTWISE_UNARY_H_
#define INFINI_OPS_CUDA_ELEMENTWISE_UNARY_H_
...
#include "cuda/elementwise/unary.cuh"
...
class CudaUnaryElementwise : public Op {
 public:
  using Op::Op;

  CudaUnaryElementwise(const Tensor input, Tensor out) : Op{input, out} {
    if (this->ndim_ == 0) {
      return;
    }
    ...
    const auto metadata_size = 2 * (shape_size + strides_size);
  ...
  void operator()(const Tensor input, Tensor out) const override {
    ...
    DispatchFunc<AllowedDataTypes, AllCudaBlockSizes>(
        ...
        [&](auto list_tag) {
          ...
          UnaryElementwiseKernel<Backend::kDeviceType, T, kBlockSize,
                                 UnaryModeOf<Op>::value>
              <<<grid_dims, block_dims, 0, cuda_stream>>>(
                  ...
                  this->output_size_, this->ndim_, this->is_out_contiguous_,
                  this->is_input_contiguous_);
        },
        "CudaUnaryElementwise::operator()");
  }
