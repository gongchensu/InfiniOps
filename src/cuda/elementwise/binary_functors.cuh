#ifndef INFINI_OPS_CUDA_ELEMENTWISE_BINARY_FUNCTORS_CUH_
#define INFINI_OPS_CUDA_ELEMENTWISE_BINARY_FUNCTORS_CUH_
...
struct CudaBinaryFunctor {
  template <typename T>
  __device__ __forceinline__ static T Apply(const T& lhs, const T& rhs) {
 ...
 private:
  ...
  __device__ __forceinline__ static ComputeType ApplyCompute(
      const ComputeType& lhs, const ComputeType& rhs) {
  ...
  template <typename ComputeType>
  __device__ __forceinline__ static ComputeType ApplyRemainder(
      const ComputeType& lhs, const ComputeType& rhs) {
    if constexpr (std::is_floating_point_v<ComputeType>) {
      auto remainder = fmod(lhs, rhs);
